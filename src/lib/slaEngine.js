...
// ── Compute the three SLA deadlines from the new priority logic ──────────────
// Call this on incident creation (or when priority/owr changes).
// Returns an object with acknowledgement_deadline, make_safe_deadline, restoration_deadline.
export function computePriorityDeadlines(createdAt, priority, isOWR, caApprovalDate) {
  ...
  if (priority === "P1") {
    ...
    // If created after 17:00, use next business day start
    ...
    // P2 restoration: created + 28 days
    restoration_deadline = addDays(created, 28).toISOString();
  }
...
// ── Update restoration deadline after CA approval (OWR incidents) ─────────────
export function computeOWRRestorationDeadline(caApprovalDate) {
  if (!caApprovalDate) return null;
