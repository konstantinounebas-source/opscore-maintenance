...
export default function IncidentWorkflow({ incident, incidentId, onRefresh }) {
  ...
  const digitalFMPI = fmpiSubmissions.find(s => (s.status === "Submitted" || s.status === "Approved") && s.form_data?.fmpi_submission_method !== "Manual");
  const manualFMPI = fmpiSubmissions.find(s => (s.status === "Submitted" || s.status === "Approved") && s.form_data?.fmpi_submission_method === "Manual");
  const hasFMPISubmitted = !!(digitalFMPI || manualFMPI);
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-3">
        ...
        {workflowState !== "Awaiting_CR_OMPI" && (
          <div className={`rounded-lg border overflow-hidden ${
            ...
            {/* Action buttons row */}
            <div className={`flex gap-2 px-3 py-2 border-t flex-wrap ${hasFMPISubmitted ? "border-green-100 bg-green-50/40" : "border-slate-100 bg-slate-50/40"}`}>
              {/* Digital FMPI button */}
              {digitalFMPI ? (
                <Button
                  size="sm"
                ...
                <Button
                  size="sm"
                  variant={!hasFMPISubmitted ? "default" : "outline"}
              ...
              {/* No FMPI at all */}
              {!hasFMPISubmitted && (
                <p className="w-full text-xs text-slate-400 text-center pt-1">No FMPI submitted yet</p>
              )}
            </div>
          </div>
        )}
