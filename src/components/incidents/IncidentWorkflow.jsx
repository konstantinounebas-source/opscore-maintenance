...
export default function IncidentWorkflow({ incident, incidentId, onRefresh }) {
  ...
  const addAudit = async (action, details, extra = {}, person = "") => {
    const user = await base44.auth.me();
    const auditUser = person || user?.email;
    await base44.entities.IncidentAuditTrail.create({ incident_id: incidentId, action, details, user: auditUser, ...extra });
  };

  const updateIncidentFlag = async (flagKey, auditAction, auditDetails, auditExtra = {}, person = "") => {
    await base44.entities.Incidents.update(incidentId, { [flagKey]: true });
    await addAudit(auditAction, auditDetails, auditExtra, person);
  ...
  const handleCorrective = async () => {
    ...
    await updateIncidentFlag("corrective_done", "Corrective WO Created", "Corrective Work Order created", { comment: formData.notes || "" }, formData.person);
    setActiveModal(null);
    setFormData({});
  };
  ...
  const handleFinalise = async () => {
    await updateIncidentFlag("finalise_done", "FMPI Finalised", "FMPI finalised and ready for closure", { comment: formData.notes || "" }, formData.person);
    if (incident.is_owr) {
      await addAudit("CA Approval Required", "OWR incident — CA Approval needed before closure", {}, formData.person);
    }
    setActiveModal(null);
    setFormData({});
  };
