        ...
        <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <SelectContent>
                  {activeCatalog.map(c => (
                    <SelectItem key={c.id} value={c.id}>
                      ...
                      {c.child_code ? ` — ${c.child_code}` : ""}
                      {c.child_category ? ` (${c.child_category})` : ""}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Input value={form.child_id} onChange={e => set("child_id", e.target.value)}
                className={selectedCatalogId ? "bg-slate-50 text-slate-600" : ""} />
            </div>
            <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            ...
            <Select value={form.parent_asset_id || "_none"} onValueChange={v => set("parent_asset_id", v === "_none" ? "" : v)}>
              <SelectTrigger>
              ...
              <SelectContent>
                ...
                {parentAssets.map(a => (
                  <SelectItem key={a.id} value={a.id}>{a.asset_id || a.asset_name || a.id}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
