...
export default function AssignmentTable({ assignments, assetsMap, onSelect, selectedId, onEdit, onRemove }) {
  ...
  const filtered = useMemo(() => {
    ...
    return enriched.filter(a =>
      !q ||
      a._asset.asset_id?.toLowerCase().includes(q) ||
      a._asset.asset_name?.toLowerCase().includes(q) ||
      a._asset.city?.toLowerCase().includes(q) ||
        ...
        <table className="w-full text-sm min-w-[700px]">
          <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
            <tr>
              ...
              <Th label="Asset Name" k="asset_name" />
          ...
          <tbody className="divide-y divide-slate-100">
            {sorted.length === 0 && (
              <tr><td colSpan={11} className="text-center py-10 text-slate-400 text-sm">No assignments for this week.</td></tr>
            )}
            {sorted.map(a => {
              ...
              return (
                <tr key={a.id}
                  ...
                  <td className="px-3 py-2 text-xs text-slate-900 max-w-[140px] truncate">{a._asset.asset_name || "—"}</td>
                  ...
                  <td className="px-3 py-2 text-xs text-slate-600 max-w-[100px] truncate">{a.team_name || "—"}</td>
                  ...
                  <td className="px-3 py-2">
                    <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                      ...
                      {a.source_work_order_id && (
                        <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => navigate(`/WorkOrders`)} title="Open Work Order">
