...
import { Plus, Pencil, MapPin, ExternalLink, Calendar, AlertTriangle, Wrench } from "lucide-react";
...
export default function AssetDetailPanel({ asset, assignment, latestIncident, latestWorkOrder, selectedWeek, onAssign, onEditAssignment }) {
  ...
  if (!asset) {
    return (
      <div className="flex flex-col items-center justify-center h-40 text-slate-400 bg-slate-50 rounded-lg border border-slate-200">
    ...
    <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
      ...
      <div className="bg-slate-900 text-white px-4 py-3 flex items-start justify-between gap-2">
        <div>
          ...
          <div className="text-sm font-semibold mt-0.5">{asset.asset_name}</div>
          {asset.location_address && <div className="text-xs text-slate-400 mt-0.5 truncate">{asset.location_address}</div>}
        </div>
        <Button
          size="sm"
          variant="ghost"
          className="text-slate-300 hover:text-white hover:bg-slate-700 h-7 text-xs shrink-0"
      ...
      <div className="px-4 py-3 border-b border-slate-100">
        ...
        <FieldRow label="Next Inspection" value={asset.next_inspection_date ? format(new Date(asset.next_inspection_date), "MMM d, yyyy") : null} />
      </div>

      {/* Assignment for selected week */}
      <div className="px-4 py-3 border-b border-slate-100">
        ...
        {assignment && (
          <div className="space-y-1.5">
            ...
            {assignment.notes && <div className="text-xs text-slate-500 italic mt-1">{assignment.notes}</div>}
          </div>
        )}
      </div>
      ...
      {latestIncident && (
        <div className="px-4 py-3 border-b border-slate-100">
          <div className="flex items-center justify-between mb-1">
            ...
            <Button size="sm" variant="ghost" className="h-6 text-xs text-indigo-600" onClick={() => navigate(`/IncidentDetail?id=${latestIncident.id}`)}>
              <ExternalLink className="w-3 h-3" />
            </Button>
          </div>
          <div className="text-xs font-medium text-slate-800">{latestIncident.incident_id} — {latestIncident.title}</div>
          <div className="text-xs text-slate-500 mt-0.5">{latestIncident.status} · {latestIncident.priority}</div>
        </div>
      )}
