...
export default function MobileInspectionForm({ token, incident, asset, existingSubmission, onSubmitted }) {
  ...
  const generateAndUploadPDF = async (submissionId, token) => {
    try {
      // Retry PDF generation (up to 3 attempts)
      let pdfData = null;
      ...
      if (!pdfData?.html) {
        console.warn('PDF generation returned no HTML - skipping');
        return;
      }
    ...
    } catch (err) { console.error('PDF generation/upload failed (non-blocking):', err); }
  };
  ...
  return (
    <div className="space-y-4 pb-10">
      ...
      <div className="flex gap-3">
        ...
        <Button className="flex-1 h-12 gap-2 bg-blue-700 hover:bg-blue-800" onClick={() => submit('Submitted')} disabled={saving || submitting || pdfGenerating}>
